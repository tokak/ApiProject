...
namespace ApiProject.WebUI.Controllers
{
    public class MessageController : Controller
    {
        ...
        public async Task<IActionResult> MessageList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7149/api/Messages/GetList");
        ...
        public async Task<IActionResult> CreateMessage(CreateMessageDto createMessageDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7149/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            await client.DeleteAsync("https://localhost:7149/api/Messages?id=" + id);
            return RedirectToAction("MessageList");
        }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7149/api/Messages/GetMessage?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateMessage(UpdateMessageDto updateMessageDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7149/api/Messages/", stringContent);
            return RedirectToAction("MessageList");
        }
        ...
        public async Task<IActionResult> AsnwerMessageWithOpenAI(int id, string prompt)
        {
            ...
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
            var value = JsonConvert.DeserializeObject<GetMessageByIdDto>(jsonData);
            prompt = value.Details;

            prompt = $"Sen bir restoran için kullanıcıların göndermiş oldukları mesajları detaylı ve olabildiğince olumlu, müşteri memnunyeti gözeten cevaplar veren bir yapay zeka aracısın." +
                $" Amacımız kullanıcı tarafından gönderilen mesajlara en olumlu ve mantıklı cevapları sunabilmek mesajların kısa ve öz olsun.Kullanıcıdan gönderilen mesaj : {prompt}}}";
            var geminiMessage = await GeminiHelper.GenerateRecipeAsync(prompt);
            ViewBag.answerAI = geminiMessage;
        ...
        public async Task<IActionResult> SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            var jsonData = JsonConvert.SerializeObject(createMessageDto);
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client2.PostAsync("https://localhost:7149/api/Messages", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("MessageList");
            }
