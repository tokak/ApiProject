...
namespace ApiProject.WebUI.Models
{
    public class ChatHub : Hub
    {
        ...
        private const string modelName = "DeepSeek-V3.1-Terminus";
        ...
        public async Task SendMessage(string userMessage)
        {
            ...
            await StreamDeepSeek(history, Context.ConnectionAborted);
        }

        private async Task StreamDeepSeek(List<Dictionary<string, string>> history, CancellationToken cancellationToken)
        {
            ...
            while (!reader.EndOfStream && !cancellationToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    {
                        sb.Append(delta);
                        await Clients.Caller.SendAsync("ReceiveToken", delta);
                    }
                }
                catch
                {
                    // parse hatası olursa sessiz geç
                }
            }
